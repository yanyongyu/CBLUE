...
CLS_MODEL_CLASS = {
    ...
    "bert-adapter": adapter(BertForSequenceClassification),
...
def main():
    ...
    parser.add_argument(
        "--do_aug", default=1, type=int, help="whether do data augment."
    )
    ...
    logger = init_logger(
        os.path.join(args.output_dir, f"{args.task_name}_{args.model_name}.log")
    )
    ...
    if args.do_train:
        ...
        data_processor = CDNDataProcessor(
            root=args.data_dir,
        ...
        (
            train_samples,
            recall_orig_train_samples,
            recall_orig_train_samples_scores,
        ) = data_processor.get_train_sample(dtype="cls", do_augment=args.do_aug)
        (
            eval_samples,
            recall_orig_eval_samples,
            recall_orig_train_samples_scores,
        ) = data_processor.get_dev_sample(dtype="cls", do_augment=args.do_aug)
        if data_processor.recall:
            logger.info("first recall score: %s", data_processor.recall)

        train_dataset = CDNDataset(
            train_samples, data_processor, dtype="cls", mode="train"
        )
        eval_dataset = CDNDataset(
            eval_samples, data_processor, dtype="cls", mode="eval"
        )

        model = CDNForCLSModel(
            model_class,
            encoder_path=os.path.join(args.model_dir, args.model_name),
            num_labels=data_processor.num_labels_cls,
        )
        cls_model_class = CLS_MODEL_CLASS[args.model_type]
        trainer = CDNForCLSTrainer(
            args=args,
            model=model,
            ...
            logger=logger,
            recall_orig_eval_samples=recall_orig_eval_samples,
        ...
        model = CDNForCLSModel(
            ...
            encoder_path=os.path.join(
                args.output_dir, f"checkpoint-{best_step}"
            ),
            num_labels=data_processor.num_labels_cls,
        )
        model.load_state_dict(
            torch.load(
                os.path.join(
                    args.output_dir, f"checkpoint-{best_step}", "task_model.pt"
                )
            )
        )
        tokenizer = tokenizer_class.from_pretrained(
            os.path.join(args.output_dir, f"checkpoint-{best_step}")
        )
        torch.save(
            model.state_dict(),
            os.path.join(args.output_dir, "task_model_cls.pt"),
        )
        ...
        tokenizer.save_vocabulary(
            save_directory=os.path.join(args.output_dir, "cls")
        )
        logger.info(
            "Saving models checkpoint to %s",
            os.path.join(args.output_dir, "cls"),
        )
        ...
        train_samples = data_processor.get_train_sample(
            dtype="num", do_augment=1
        )
        eval_samples = data_processor.get_dev_sample(dtype="num")
        train_dataset = CDNDataset(
            train_samples, data_processor, dtype="num", mode="train"
        )
        eval_dataset = CDNDataset(
            eval_samples, data_processor, dtype="num", mode="eval"
        )

        cls_model_class = CLS_MODEL_CLASS[args.model_type]
        model = cls_model_class.from_pretrained(
            os.path.join(args.model_dir, args.model_name),
            num_labels=data_processor.num_labels_num,
        )
        trainer = CDNForNUMTrainer(
            args=args,
            model=model,
            data_processor=data_processor,
            tokenizer=tokenizer,
    ...
    if args.do_predict:
        ...
        (
            test_samples,
            recall_orig_test_samples,
            recall_orig_test_samples_scores,
        ) = data_processor.get_test_sample(dtype="cls")

        test_dataset = CDNDataset(
            test_samples, data_processor, dtype="cls", mode="test"
        )
        cls_model_class = CLS_MODEL_CLASS[args.model_type]

        model = CDNForCLSModel(
            model_class,
            encoder_path=os.path.join(args.output_dir, "cls"),
            num_labels=data_processor.num_labels_cls,
        )
        model.load_state_dict(
            torch.load(os.path.join(args.output_dir, "task_model_cls.pt"))
        )
        trainer = CDNForCLSTrainer(
            args=args,
            model=model,
        ...
        test_dataset = CDNDataset(
            test_samples, data_processor, dtype="num", mode="test"
        )
        model = cls_model_class.from_pretrained(
            os.path.join(args.output_dir, "num"),
            num_labels=data_processor.num_labels_num,
        )
        trainer = CDNForNUMTrainer(
            args=args,
