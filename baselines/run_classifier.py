...
def main():
    ...
    if args.do_train:
        ...
        if args.task_name in ["ee", "ctc"]:
            ...
            train_dataset = dataset_class(
                train_samples, data_processor, mode="train"
            )
            eval_dataset = dataset_class(
                eval_samples, data_processor, mode="eval"
            )
    ...
    if args.do_predict:
        ...
        if args.task_name in ["ee", "ctc"]:
            ...
            test_dataset = dataset_class(
                test_samples, data_processor, mode="test"
            )

        model = model_class.from_pretrained(
            args.output_dir, num_labels=data_processor.num_labels
        )
        trainer = trainer_class(
            args=args,
            model=model,
