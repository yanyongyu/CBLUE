...
def main():
    ...
    if args.do_train:
        ...
        torch.save(
            model.state_dict(),
            os.path.join(args.output_dir, "task_model_er.pt"),
        )
        ...
        tokenizer.save_vocabulary(
            save_directory=os.path.join(args.output_dir, "er")
        )
        logger.info(
            "Saving models checkpoint to %s",
            os.path.join(args.output_dir, "er"),
        )
        ...
        tokenizer.add_special_tokens(
            {"additional_special_tokens": ["<s>", "</s>", "<o>", "</o>"]}
        )
        ...
        torch.save(
            model.state_dict(),
            os.path.join(args.output_dir, "task_model_re.pt"),
        )
        ...
        tokenizer.save_vocabulary(
            save_directory=os.path.join(args.output_dir, "re")
        )
        logger.info(
            "Saving models checkpoint to %s",
            os.path.join(args.output_dir, "re"),
        )
    ...
    if args.do_predict:
        ...
        model = ERModel(
            model_class, encoder_path=os.path.join(args.output_dir, "er")
        )
        model.load_state_dict(
            torch.load(os.path.join(args.output_dir, "task_model_er.pt"))
        )
        trainer = ERTrainer(
            args=args,
            model=model,
        ...
        model.load_state_dict(
            torch.load(os.path.join(args.output_dir, "task_model_re.pt"))
        )
        trainer = RETrainer(
            args=args,
            model=model,
        ...
        trainer.predict(
            test_samples=test_samples, model=model, re_dataset_class=REDataset
        )
