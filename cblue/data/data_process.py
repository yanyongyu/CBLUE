...
from cblue.utils import str_q2b, load_dict, load_json, write_dict
...
class EEDataProcessor(object):
    def __init__(self, root, is_lower=True, no_entity_label="O"):
        ...
        self.label_map_cache_path = os.path.join(
            self.task_data_dir, "CMeEE_label_map.dict"
        )
        self.label2id = None
        self.id2label = None
    ...
    def extract_result(self, results, test_input):
        ...
        for j in range(len(results)):
            ...
            for i, (char, tag) in enumerate(zip(text, results[j])):
                ...
                if tag[0] == "B":
                    if entity_name != "":
                        x = {a: flag.count(a) for a in flag}
        ...
        with open(path, "r", encoding="utf8") as f:
            ...
            for line in lines:
                ...
                for spo in data["spo_list"]:
                    ...
                    o_dict[spo["object"]["@value"]] = spo["object_type"][
                        "@value"
                    ]
        ...
        with open(self.schema_path, "r", encoding="utf8") as f:
            ...
            for line in lines:
                ...
                predicate_list.append(
                    data["predicate"] + "|" + data["object_type"]
                )
                pre_sub_obj[data["predicate"] + "|" + data["object_type"]] = [
                    data["subject_type"],
                    data["object_type"],
                ]
        ...
        for sub in data["sub_list"]:
            for obj in data["obj_list"]:
                ...
                result.append(
                    {
                        "text": text3,
                        "flag": (sub_flag[0], obj_flag[0]),
                        "spo_list": {"subject": sub, "object": obj},
                    }
                )
...
class ERDataProcessor(object):
    ...
    def _extract_entity(
        self, start_logits, end_logits, text_start_id, text_end_id
    ):
        # logits: seq
    ...
    def extract_arg(
        ...
        arg_tuple = self._extract_entity(
            start_logits, end_logits, text_start_id, text_end_id
        )
...
class CDNDataProcessor(object):
    def __init__(self, root, recall_k=200, negative_sample=3):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "CHIP-CDN_train.json"
        )
        ...
        self.label_path = os.path.join(
            self.task_data_dir, "国际疾病分类 ICD-10北京临床版v601.xlsx"
        )
    ...
    def get_train_sample(self, dtype="cls", do_augment=1):
        ...
        if dtype == "cls":
            (
                outputs,
                recall_orig_samples,
                recall_orig_samples_scores,
            ) = self._get_cls_samples(
                orig_samples=samples, mode="train", do_augment=do_augment
            )
            return outputs, recall_orig_samples, recall_orig_samples_scores
        else:
            outputs = self._get_num_samples(
                orig_sample=samples, is_predict=False
            )
        return outputs
    ...
    def get_dev_sample(self, dtype="cls", do_augment=1):
        ...
        if dtype == "cls":
            (
                ...
                orig_samples=samples, mode="eval", do_augment=do_augment
            )
            return outputs, recall_orig_samples, recall_orig_samples_scores
        else:
            outputs = self._get_num_samples(
    ...
    def get_test_sample(self, dtype="cls"):
        ...
        if dtype == "cls":
            (
                ...
                recall_orig_samples,
                recall_orig_samples_scores,
            ) = self._get_cls_samples(orig_samples=samples, mode="test")
            return outputs, recall_orig_samples, recall_orig_samples_scores
        else:
            outputs = self._get_num_samples(
                orig_sample=samples, is_predict=True
            )
        return outputs
    ...
    def _pre_process(self, path, is_predict=False):
        ...
        for sample in samples:
            ...
            if is_predict:
                ...
                label = self._process_single_sentence(
                    sample["normalized_result"], mode="label"
                )
                outputs["label"].append(
                    [label_ for label_ in label.split("##")]
                )
                outputs["text"].append(text)
        return outputs
    ...
    def _save_cache(self, outputs, recall_orig_samples, mode="train"):
        ...
        cache_df.to_csv(
            os.path.join(self.task_data_dir, f"{mode}_samples.csv"), index=False
        )
        recall_orig_cache_df = pd.DataFrame(recall_orig_samples)
        recall_orig_cache_df["label"] = recall_orig_cache_df.label.apply(
            lambda x: " ".join([str(i) for i in x])
        )
        recall_orig_cache_df[
            "recall_label"
        ] = recall_orig_cache_df.recall_label.apply(
            lambda x: " ".join([str(i) for i in x])
        )
        recall_orig_cache_df.to_csv(
            os.path.join(self.task_data_dir, f"{mode}_recall_orig_samples.csv"),
            index=False,
        )
    ...
    def _load_cache(self, mode="train"):
        ...
        train_cache_df = pd.read_csv(
            os.path.join(self.task_data_dir, f"{mode}_samples.csv")
        )
        ...
        train_recall_orig_cache_df = pd.read_csv(
            os.path.join(self.task_data_dir, f"{mode}_recall_orig_samples.csv")
        )
        recall_orig_samples["text"] = train_recall_orig_cache_df[
            "text"
        ].values.tolist()
        recall_orig_samples["label"] = train_recall_orig_cache_df[
            "label"
        ].values.tolist()
        recall_orig_samples["recall_label"] = train_recall_orig_cache_df[
            "recall_label"
        ].values.tolist()
        recall_orig_samples["label"] = [
            [
                int(label)
                for label in str(recall_orig_samples["label"][i]).split()
            ]
            for i in range(len(recall_orig_samples["label"]))
        ]
        recall_orig_samples["recall_label"] = [
            [
                int(label)
                for label in str(recall_orig_samples["recall_label"][i]).split()
            ]
            for i in range(len(recall_orig_samples["recall_label"]))
        ]
        recall_samples_scores = np.load(
            os.path.join(self.task_data_dir, f"{mode}_recall_score.npy")
        )
    ...
    def _get_cls_samples(self, orig_samples, mode="train", do_augment=1):
        if (
            ...
            and os.path.exists(
                os.path.join(
                    self.task_data_dir, f"{mode}_recall_orig_samples.csv"
                )
            )
        ):
            (
                outputs,
                recall_orig_samples,
                recall_samples_scores,
            ) = self._load_cache(mode=mode)
            return outputs, recall_orig_samples, recall_samples_scores
        ...
        if mode == "train":
            ...
            for text, orig_label, recall_label in zip(
                texts, labels, recall_samples_idx
            ):
    ...
    def _recall(self, texts):
        ...
        for i, x in tqdm.tqdm(enumerate(texts), total=len(texts)):
            ...
            x_scores_idx = np.argsort(x_scores)[
                : len(x_scores) - self.recall_k - 1 : -1
            ]  # 由大到小排序,取前K个
            x_scores = np.sort(x_scores)[
                : len(x_scores) - self.recall_k - 1 : -1
            ]
...
class CTCDataProcessor(object):
    def __init__(self, root):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "CHIP-CTC_train.json"
        )
        ...
        self.label2id = {
            label: idx for idx, label in enumerate(self.label_list)
        }
        self.id2label = {
            idx: label for idx, label in enumerate(self.label_list)
        }
    ...
    def _pre_process(self, path, is_predict=False):
        ...
        for sample in samples:
            outputs["text"].append(
                "\002".join([t for t in list(sample["text"].lower())])
            )
...
class STSDataProcessor(object):
    def __init__(self, root):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "CHIP-STS_train.json"
        )
...
class QQRDataProcessor(object):
    def __init__(self, root):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "KUAKE-QQR_train.json"
        )
...
class QICDataProcessor(object):
    def __init__(self, root):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "KUAKE-QIC_train.json"
        )
...
class QTRDataProcessor(object):
    def __init__(self, root):
        ...
        self.train_path = os.path.join(
            self.task_data_dir, "KUAKE-QTR_train.json"
        )
