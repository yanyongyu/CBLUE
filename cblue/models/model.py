...
class ERModel(nn.Module):
    def __init__(self, encoder_class, encoder_path):
        ...
        self.sub_startlayer = nn.Linear(
            in_features=self.encoder.config.hidden_size, out_features=1
        )
        self.sub_endlayer = nn.Linear(
            in_features=self.encoder.config.hidden_size, out_features=1
        )
        self.obj_startlayer = nn.Linear(
            in_features=self.encoder.config.hidden_size, out_features=1
        )
        self.obj_endlayer = nn.Linear(
            in_features=self.encoder.config.hidden_size, out_features=1
        )
    ...
    def forward(
        ...
        return (
            sub_start_idx.squeeze(-1),
            sub_end_idx.squeeze(-1),
            obj_start_idx.squeeze(-1),
            obj_end_idx.squeeze(-1),
        )


class REModel(nn.Module):
    def __init__(
        self, tokenizer, encoder_class, encoder_path, num_labels, config=None
    ):
        ...
        self.classifier = nn.Linear(
            in_features=self.encoder.config.hidden_size * 2,
    ...
    def forward(
        ...
        entity_hidden_state = torch.Tensor(
            batch_size, 2 * hidden_size
        )  # batch, 2*hidden
        # flag: batch, 2
        for i in range(batch_size):
            sub_start_idx, obj_start_idx = flag[i, 0], flag[i, 1]
            start_entity = last_hidden_state[i, sub_start_idx, :].view(
                hidden_size,
            )  # s_start: hidden,
            end_entity = last_hidden_state[i, obj_start_idx, :].view(
                hidden_size,
            )  # o_start: hidden,
            entity_hidden_state[i] = torch.cat(
                [start_entity, end_entity], dim=-1
            )
...
class CDNForCLSModel(nn.Module):
    ...
    def forward(
        ...
        avg_hidden_states = torch.mean(
            (last_hidden_states + first_hidden_states), dim=1
        )
