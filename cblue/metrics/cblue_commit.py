...
def sts_commit_prediction(dataset, preds, output_dir, id2label):
    ...
    with open(
        os.path.join(output_dir, "CHIP-STS_test.json"), "w", encoding="utf-8"
    ) as f:
...
def qic_commit_prediction(dataset, preds, output_dir, id2label):
    ...
    for item in zip(ids, text1, label):
        tmp_dict = {"id": item[0], "query": item[1], "label": id2label[item[2]]}
        pred_result.append(tmp_dict)
    with open(
        os.path.join(output_dir, "KUAKE-QIC_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))
...
def qtr_commit_prediction(dataset, preds, output_dir, id2label):
    ...
    for item in zip(ids, text1, text2, label):
        ...
        pred_result.append(tmp_dict)
    with open(
        os.path.join(output_dir, "KUAKE-QTR_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))
...
def qqr_commit_prediction(dataset, preds, output_dir, id2label):
    ...
    for item in zip(ids, text1, text2, label):
        ...
        pred_result.append(tmp_dict)
    with open(
        os.path.join(output_dir, "KUAKE-QQR_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))
...
def ctc_commit_prediction(dataset, preds, output_dir, id2label):
    ...
    for item in zip(ids, text1, label):
        ...
        pred_result.append(tmp_dict)
    with open(
        os.path.join(output_dir, "CHIP-CTC_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))
...
def ee_commit_prediction(dataset, preds, output_dir):
    ...
    for item in zip(orig_text, preds):
        ...
        pred_result.append(tmp_dict)
    with open(
        os.path.join(output_dir, "CMeEE_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))


def cdn_commit_prediction(
    text, preds, num_preds, recall_labels, recall_scores, output_dir, id2label
):
    text1 = text

    pred_result = []
    active_indices = preds >= 0.5
    for text, active_indice, pred, num, recall_label, recall_score in zip(
        text1, active_indices, preds, num_preds, recall_labels, recall_scores
    ):
        ...
        if len(final_pred):
            ...
            if num == 1:
                ...
                tmp_dict["normalized_result"].extend(
                    recall_label[:sum_].tolist()
                )
            tmp_dict["normalized_result"] = [
                id2label[idx] for idx in tmp_dict["normalized_result"]
            ]
    ...
    with open(
        os.path.join(output_dir, "CHIP-CDN_test.json"), "w", encoding="utf-8"
    ) as f:
        f.write(json.dumps(pred_result, indent=2, ensure_ascii=False))
