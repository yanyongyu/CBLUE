...
from cblue.utils import ProgressBar, TokenRematch, seed_everything
from cblue.metrics import (
    ee_metric,
    er_metric,
    re_metric,
    ctc_metric,
    ...
    ctc_commit_prediction,
    qic_commit_prediction,
    qqr_commit_prediction,
    qtr_commit_prediction,
    sts_commit_prediction,
)
...
class Trainer(object):
    ...
    def train(self):
        ...
        logger.info(
            "Training Stop! The best step %s: %s", best_step, best_score
        )
        if args.device == "cuda":
...
class EETrainer(Trainer):
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            if preds is None:
                ...
                preds = np.append(
                    preds, active_logits.detach().cpu().numpy(), axis=0
                )
    ...
    def predict(self, model, test_dataset):
        ...
        ee_commit_prediction(
            ...
            preds=predicts,
            output_dir=args.result_output_dir,
        )

    def _save_checkpoint(self, model, step):
        output_dir = os.path.join(
            self.args.output_dir, "checkpoint-{}".format(step)
        )
    ...
    def _save_best_checkpoint(self, best_step):
        ...
        torch.save(
            self.args,
            os.path.join(self.args.output_dir, "training_args.bin"),
        )
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            if preds is None:
                ...
                eval_labels = np.append(
                    eval_labels, labels.detach().cpu().numpy(), axis=0
                )
...
class ERTrainer(Trainer):
    ...
    def training_step(self, model, item):
        ...
        active_index = attention_mask.view(-1) == 1
        sub_start_loss = self.cal_loss(
            sub_start_logits, sub_start_label, active_index
        )
        sub_end_loss = self.cal_loss(
            sub_end_logits, sub_end_label, active_index
        )
        obj_start_loss = self.cal_loss(
            obj_start_logits, obj_start_label, active_index
        )
        obj_end_loss = self.cal_loss(
            obj_end_logits, obj_end_label, active_index
        )
    ...
    def cal_loss(self, logits, labels, active_index):
        ...
        active_logits = logits.view(-1)[active_index]
        return self.loss_fn(
            active_logits.float()[1:-1], active_labels.float()[1:-1]
        )
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            sub_end_preds.extend(
                (sub_end_logits.detach().view(-1) >= 0.5)
                .cpu()
                .long()[active_index]
            )
            obj_start_preds.extend(
                (obj_start_logits.detach().view(-1) >= 0.5)
                .cpu()
                .long()[active_index]
            )
            obj_end_preds.extend(
                (obj_end_logits.detach().view(-1) >= 0.5).cpu()[active_index]
            )

            sub_start_trues.extend(
                sub_start_label.detach().cpu().view(-1)[active_index].tolist()
            )
            sub_end_trues.extend(
                sub_end_label.detach().cpu().view(-1)[active_index].tolist()
            )
            obj_start_trues.extend(
                obj_start_label.detach().cpu().view(-1)[active_index].tolist()
            )
            obj_end_trues.extend(
                obj_end_label.detach().cpu().view(-1)[active_index].tolist()
            )

        s_start_p, s_start_r, s_start_f1, _ = er_metric(
            sub_start_preds, sub_start_trues
        )
        s_end_p, s_end_r, s_end_f1, _ = er_metric(sub_end_preds, sub_end_trues)
        o_start_p, o_start_r, o_start_f1, _ = er_metric(
            obj_start_preds, obj_start_trues
        )
    ...
    def predict(self, test_dataset, model):
        ...
        logger.info("Num samples %d", num_examples)
        with open(
            os.path.join(args.output_dir, "CMeIE_test.jsonl"),
            "w",
            encoding="utf-8",
        ) as f:
            for step, item in enumerate(test_dataloader):
                ...
                with torch.no_grad():
                    ...
                    sub_arg_list = self.data_processor.extract_arg(
                        sub_start_logits.view(-1),
                        sub_end_logits.view(-1),
                        text_start_id,
                        text_end_id,
                        text,
                        text_mapping,
                    )
                    obj_arg_list = self.data_processor.extract_arg(
                        obj_start_logits.view(-1),
                        obj_end_logits.view(-1),
                        text_start_id,
                        text_end_id,
                        text,
                        text_mapping,
                    ...
                    result = {
                        "text": text,
                        "sub_list": sub_arg_list,
                        "obj_list": obj_arg_list,
                    }
    ...
    def _save_checkpoint(self, model, step):
        ...
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        torch.save(
            model.state_dict(), os.path.join(output_dir, "task_model.pt")
        )
        self.logger.info("Saving models checkpoint to %s", output_dir)
...
class RETrainer(Trainer):
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            if preds is None:
                ...
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                eval_labels = np.append(
                    eval_labels, label.detach().cpu().numpy(), axis=0
                )
    ...
    def predict(self, test_samples, model, re_dataset_class):
        ...
        logger.info("***** Running prediction *****")
        with open(
            os.path.join(args.result_output_dir, "CMeIE_test.jsonl"),
            "w",
            encoding="utf-8",
        ) as f:
            for data in test_samples:
                ...
                temp_re_dataset = re_dataset_class(
                    outputs,
                ...
                with torch.no_grad():
                    for item in temp_re_dataset:
                        ...
                        input_ids, token_type_ids, attention_mask, flag = (
                ...
                for i in range(len(temp_re_dataset)):
                    if logits[i] > 0:
                        spo_list[i][
                            "predicate"
                        ] = self.data_processor.id2predicate[logits[i]]
                ...
                for spo in spo_list:
                    if "predicate" in spo.keys():
                        ...
                        tmp["subject_type"] = self.data_processor.pre_sub_obj[
                            spo["predicate"]
                        ][0]
                        tmp["object"] = {"@value": spo["object"]}
                        tmp["object_type"] = {
                            "@value": self.data_processor.pre_sub_obj[
                                spo["predicate"]
                            ][1]
                        }
    ...
    def predict(self, test_dataset, model):
        ...
        for step, item in enumerate(test_dataloader):
            ...
            if preds is None:
                ...
                preds = np.append(
                    preds,
                    logits.detach().softmax(-1)[:, 1].cpu().numpy(),
                    axis=0,
                )
        ...
        np.save(
            os.path.join(args.result_output_dir, f"cdn_test_preds.npy"), preds
        )
        return preds
...
class CDNForNUMTrainer(Trainer):
    ...
    def _save_best_checkpoint(self, best_step):
        ...
        torch.save(
            self.args,
            os.path.join(
                os.path.join(self.args.output_dir, "num"),
                "training_args.bin",
            ),
        )
        self.tokenizer.save_vocabulary(
            save_directory=os.path.join(self.args.output_dir, "num")
        )
        self.logger.info(
            "Saving models checkpoint to %s",
            os.path.join(self.args.output_dir, "num"),
        )
